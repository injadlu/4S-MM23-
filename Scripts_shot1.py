...
def training_loop(lowshot_dataset,novel_test_feats, num_classes, params, batchsize=1000, maxiters=1000, nTimes = 0):
    if os.path.exists('Model_SHOT1/' + params.name + '/LayerBestModel_' + str(params.beta) + '_' + str(params.lamda) + '/') == False:
        os.makedirs('Model_SHOT1/' + params.name + '/LayerBestModel_' + str(params.beta) + '_' + str(params.lamda) + '/')
    if os.path.exists('Model_SHOT1/' + params.name + '/' + str(nTimes) + '_' + str(params.beta) + '_' + str(params.lamda)) == False:
        os.makedirs('Model_SHOT1/' + params.name + '/' + str(nTimes) + '_' + str(params.beta) + '_' + str(params.lamda))
    ...
    for epoch in range(maxiters):
        ...
        if reload_model == True:
            ...
            if str(tmp_epoach) in recode_reload:
                ...
                if tmp_rate < 1e-4:
                    if best_ACC <= 0.2:
                        return
                    new_path = 'Model_SHOT1/' + params.name + '/LayerBestModel_' + str(params.beta) + '_' + str(params.lamda) + '/'
                    f = open(new_path + str(nTimes) +'_save_' + str(tmp_epoach)+ '_' + str(round(best_ACC, 8)) + '.pth','w')
                    f.close()
                    shutil.rmtree('Model_SHOT1/' + params.name + '/' +  str(nTimes) + '_' + str(params.beta) + '_' + str(params.lamda))
            ...
            pretrained_model = 'Model_SHOT1/' + params.name + '/' +  str(nTimes) + '_' + str(params.beta) + '_' + str(params.lamda) +'/' + str(nTimes) +'_save_' + str(tmp_epoach)+ '_' + str(round(best_ACC, 8)) + '.pth'
        ...
        if (epoch%10==0):
            ...
            if  accs[0] > best_ACC :
                ...
                best_ACC = accs[0]
                save_file_path = 'Model_SHOT1/' + params.name + '/' +  str(nTimes) + '_' + str(params.beta) + '_' + str(params.lamda) +'/' + str(nTimes) +'_save_' + str(epoch)+ '_' + str(round(best_ACC, 8)) + '.pth'
                states = {
...
def parse_args():
    ...
    parser.add_argument('--name', default='1-shot', type=str)
...
if __name__ == '__main__':
    ...
    if os.path.exists('Model_SHOT1/' + params.name + '/LayerBestModel_' + str(params.beta) + '_' + str(params.lamda)):
        len_results = len(os.listdir('Model_SHOT1/' + params.name + '/LayerBestModel_' + str(params.beta) + '_' + str(params.lamda)))
        if len_results < end_:
            start_ = len_results
        else:
            start_ = end_
